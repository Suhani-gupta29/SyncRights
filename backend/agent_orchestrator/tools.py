...
def search_catalog(query: str) -> dict:
    ...
    # STEP 1: P1 (Abhishek) finds the track
    tracks = p1_search(q=query)
    if not tracks:
        return {"error": f"No tracks found for query: {query}"}
        
    top_track_id = tracks[0]["track_id"]
    
    # STEP 2: P2 (Kislay) finds the exact AI version match
    p2_result = p2_match(track_id=top_track_id, user_query=query)
    
    if p2_result.get("status") == "success":
        return {
            "matches": [
                {
                    "track_id": p2_result.get("track_id"),
                    "version_id": p2_result.get("matched_version_id"),
                    "title_label": p2_result.get("title_label"),
                    "version_type": p2_result.get("version_type"),
                    "ai_reasoning": p2_result.get("ai_reasoning")
                }
            ],
            "count": 1
        }
    return {"error": p2_result.get("message", "Version match failed.")}
